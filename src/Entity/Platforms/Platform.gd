...
func _process(delta):
	...
	if is_instance_valid(Collision) and is_instance_valid(EntitySprite) and \
		State.timeline.timestamp > enter_timestamp and State.timeline.timestamp < exit_timestamp:
		var pos = EntitySprite.rect_position
		var width = EntitySprite.rect_size.x * EntitySprite.rect_scale.x
		var height = EntitySprite.rect_size.y * EntitySprite.rect_scale.y
